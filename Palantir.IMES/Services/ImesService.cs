...
internal class ImesService(IHorizonLogger log, IHorizonMessaging messaging, AzureOpenAIService openai) : IHostedService
{
    ...
    private void IncomingMessage(MessagePayload payload)
    {
        ...
        var responseAsOutput = response.Replace("{", "{{").Replace("}", "}}");
        _log.Info($"Response from Azure OpenAI: {responseAsOutput}");

        _messaging.SendMessage("Results", new OutputMessage { ModelResponse = responseAsOutput }).Wait();
    }
